...
LackManagement::LackManagement(QWidget *parent) : QWidget(parent)
{
    initUI();
}

void LackManagement::initUI()
{
    this->setObjectName("lackmanagement");
...
void LackManagement::initPlan()
{
    ...
    for (int i=0; i < json_plan.size(); i++) {
        ...
        if (id > max_plan_id)
            max_plan_id = id;
        m_plan->setItem(i, LACK_P_CSTM, new QStandardItem(customer));
        m_plan->setItem(i, LACK_P_CODE, new QStandardItem(plannumber));
        m_plan->setItem(i, LACK_P_PCOD, new QStandardItem(lacknumber));
        m_plan->setItem(i, LACK_P_NAME, new QStandardItem(lackname));
        m_plan->setItem(i, LACK_P_TYPE, new QStandardItem(lacktype));
        m_plan->setItem(i, LACK_P_UNIT, new QStandardItem(lackunit));
        m_plan->setItem(i, LACK_P_QUAN, new QStandardItem(lacknum));
        m_plan->setItem(i, LACK_P_OQUA, new QStandardItem(lackenum));
        m_plan->setItem(i, LACK_P_DATE, new QStandardItem(lackdate));
        //        m_plan->setItem(i, LACK_P_IOUT, new QStandardItem(orderdate));
        //        m_plan->setItem(i, LACK_P_PDAT, new QStandardItem(ordersale));
        //        m_plan->setItem(i, LACK_P_EDAT, new QStandardItem(ordersale));
        //        m_plan->setItem(i, LACK_P_TQUA, new QStandardItem(customer));
        //        m_plan->setItem(i, LACK_P_PQUA, new QStandardItem(orderedate));
        //        m_plan->setItem(i, LACK_P_BQUA, new QStandardItem(status));
        m_plan->setItem(i, LACK_P_MARK, new QStandardItem(lackmask));
    }
}

void LackManagement::addPlan()
{
    QJsonObject obj;
    max_plan_id++;
    obj.insert("erp_productid",max_plan_id);
    obj.insert("erp_ordercustomer",m_add->item(LACK_P_CSTM,1)->text());
    obj.insert("erp_plannumber",m_add->item(LACK_P_CODE,1)->text());
    obj.insert("erp_lacknumber",m_add->item(LACK_P_PCOD,1)->text());
    obj.insert("erp_lackname",m_add->item(LACK_P_NAME,1)->text());
    obj.insert("erp_lacktype",m_add->item(LACK_P_TYPE,1)->text());
    obj.insert("erp_lackunit",m_add->item(LACK_P_UNIT,1)->text());
    obj.insert("erp_lacknum",m_add->item(LACK_P_QUAN,1)->text().toInt());
    obj.insert("erp_lackenum",m_add->item(LACK_P_OQUA,1)->text().toInt());
...
void LackManagement::sndOrder()
{
    //待添加
}

void LackManagement::tabPlanSync(QModelIndex index)
{
    QJsonObject obj = json_plan.at(index.row()).toObject();
    m_add->item(LACK_P_CSTM,1)->setText(obj.value("erp_ordercustomer").toString());
    m_add->item(LACK_P_CODE,1)->setText(obj.value("erp_plannumber").toString());
    m_add->item(LACK_P_PCOD,1)->setText(obj.value("erp_lacknumber").toString());
    m_add->item(LACK_P_NAME,1)->setText(obj.value("erp_lackname").toString());
    m_add->item(LACK_P_TYPE,1)->setText(obj.value("erp_lacktype").toString());
    m_add->item(LACK_P_UNIT,1)->setText(obj.value("erp_lackunit").toString());
    m_add->item(LACK_P_QUAN,1)->setText(QString::number(obj.value("erp_lacknum").toInt()));
    m_add->item(LACK_P_OQUA,1)->setText(QString::number(obj.value("erp_lackenum").toInt()));

    m_snd->item(LACK_P_CSTM,1)->setText(obj.value("erp_ordercustomer").toString());
    m_snd->item(LACK_P_CODE,1)->setText(obj.value("erp_plannumber").toString());
    m_snd->item(LACK_P_PCOD,1)->setText(obj.value("erp_lacknumber").toString());
    m_snd->item(LACK_P_NAME,1)->setText(obj.value("erp_lackname").toString());
    m_snd->item(LACK_P_TYPE,1)->setText(obj.value("erp_lacktype").toString());
    m_snd->item(LACK_P_UNIT,1)->setText(obj.value("erp_lackunit").toString());
    m_snd->item(LACK_P_QUAN,1)->setText(QString::number(obj.value("erp_lacknum").toInt()));
    m_snd->item(LACK_P_OQUA,1)->setText(QString::number(obj.value("erp_lackenum").toInt()));
}

void LackManagement::tabPrepSync(QModelIndex index)
{

}

void LackManagement::recvSocket(QUrl url)
{
    QString cmd = url.query();
    QString usr = url.userName();
    if (usr != "lackmanagement")
        return;
...
void LackManagement::refresh()
{
    QUrl url;
    url.setQuery("lackinfo");
    emit sendSocket(url);
}

void LackManagement::showEvent(QShowEvent *e)
{
    refresh();
    e->accept();
}
